...
with st.sidebar:
    ...
    preset = st.selectbox(
        ...
        help="Solo rellena los parámetros del mercado (a, b, c, d) y el valor "
             "de la política con los datos del enunciado. No cambia de pestaña: "
             "elegí la pestaña del ejercicio que quieras simular.",
    )
    ...
    st.markdown("---")
    # Valores por defecto de cada política según el preset (se usan en las pestañas)
    if preset == "Ej. 1 — Subsidio al transporte":
        s_default = 8.0
    else:
        s_default = 4.0
...
with tab_sub:
    ...
    with cgraf:
        # Aviso de qué mercado está cargado (resuelve la confusión preset/pestaña)
        if preset.startswith("Ej. 2"):
            nota("Atención: tenés cargados los datos del <b>Ejercicio 2 "
                 "(alquileres)</b>. Para el subsidio del enunciado, elegí "
                 "<b>Ej. 1</b> en la barra lateral.", "warn")
        p_arr, p_top = grilla_precios(a, b, c, d)
        qd = np.clip(a - b * p_arr, 0, None)
        qo = np.clip(c + d * p_arr, 0, None)
        ...
        fig = go.Figure()
        if s > 0:
            # --- Excedentes con subsidio (se calculan con la cantidad nueva Q1) ---
            # Excedente del consumidor: triángulo entre demanda y Pc, de 0 a Q1
            fig.add_trace(go.Scatter(
                x=[0, 0, R["Q1"]], y=[R["Pc"], a / b, R["Pc"]],
                fill="toself", fillcolor=COL_EC, line=dict(width=0),
                name="Excedente consumidor", hoverinfo="skip"))
            # Excedente del productor: entre Pv y la oferta, de 0 a Q1
            fig.add_trace(go.Scatter(
                x=[0, 0, R["Q1"]], y=[R["Pv"], max(-c / d, 0), R["Pv"]],
                fill="toself", fillcolor=COL_EP, line=dict(width=0),
                name="Excedente productor", hoverinfo="skip"))
        fig.add_trace(go.Scatter(x=qd, y=p_arr, name="Demanda",
                                 line=dict(color=COL_DEM, width=3)))
        ...
        if s > 0:
            ...
            # --- Puntos con LETRAS de referencia ---
            # A = precio comprador, B = precio vendedor, sobre la cantidad nueva
            fig.add_trace(go.Scatter(
                x=[R["Q1"]], y=[R["Pc"]], mode="markers+text",
                text=["A"], textposition="bottom right",
                textfont=dict(size=15, color="#dbeafe"),
                marker=dict(color=COL_DEM, size=12, line=dict(color="white", width=1)),
                name="A · Precio comprador (Pc)"))
            fig.add_trace(go.Scatter(
                x=[R["Q1"]], y=[R["Pv"]], mode="markers+text",
                text=["B"], textposition="top right",
                textfont=dict(size=15, color="#ede9fe"),
                marker=dict(color=COL_OFE2, size=12, line=dict(color="white", width=1)),
                name="B · Precio vendedor (Pv)"))
        # C = equilibrio original
        fig.add_trace(go.Scatter(
            x=[Q0], y=[P0], mode="markers+text",
            text=["C"], textposition="top center",
            textfont=dict(size=15, color="#fecaca"),
            marker=dict(color=COL_EQ, size=13, line=dict(color="white", width=1)),
...
with tab_pmax:
    ...
    with cgraf:
        # Aviso de qué mercado está cargado
        if preset.startswith("Ej. 1"):
            nota("Atención: tenés cargados los datos del <b>Ejercicio 1 "
                 "(transporte)</b>. Para el precio máximo del enunciado, elegí "
                 "<b>Ej. 2</b> en la barra lateral.", "warn")
        p_arr, p_top = grilla_precios(a, b, c, d)
        qd = np.clip(a - b * p_arr, 0, None)
        qo = np.clip(c + d * p_arr, 0, None)
        ...
        fig = go.Figure()
        if R["vinculante"]:
            Qt = R["Qt"]                       # cantidad transada (lado corto = oferta)
            Pd_en_Qt = precio_demanda(Qt, a, b)
            # Excedente del consumidor con tope: trapecio entre demanda y Pmax, de 0 a Qt
            fig.add_trace(go.Scatter(
                x=[0, 0, Qt, Qt],
                y=[ptecho, a / b, Pd_en_Qt, ptecho],
                fill="toself", fillcolor=COL_EC, line=dict(width=0),
                name="Excedente consumidor", hoverinfo="skip"))
            # Excedente del productor con tope: triángulo entre Pmax y oferta, de 0 a Qt
            fig.add_trace(go.Scatter(
                x=[0, 0, Qt], y=[ptecho, max(-c / d, 0), ptecho],
                fill="toself", fillcolor=COL_EP, line=dict(width=0),
                name="Excedente productor", hoverinfo="skip"))
        else:
            # No vinculante: excedentes del equilibrio libre
            fig.add_trace(go.Scatter(
                x=[0, 0, Q0], y=[P0, a / b, P0],
                fill="toself", fillcolor=COL_EC, line=dict(width=0),
                name="Excedente consumidor", hoverinfo="skip"))
        ...
        if R["vinculante"]:
            # Pérdida de eficiencia (triángulo entre Qt y Q0)
            fig.add_trace(go.Scatter(
                x=[R["Qt"], Q0, R["Qt"]],
                y=[precio_demanda(R["Qt"], a, b), P0, precio_oferta(R["Qt"], c, d)],
                fill="toself", fillcolor=COL_DWL, line=dict(width=0),
                name="Pérdida de eficiencia", hoverinfo="skip"))
            # A = lo que se ofrece (lado corto), B = lo que se demanda, al precio techo
            fig.add_trace(go.Scatter(
                x=[R["Qo"]], y=[ptecho], mode="markers+text",
                text=["A"], textposition="bottom right",
                textfont=dict(size=15, color="#bbf7d0"),
                marker=dict(color=COL_OFE, size=12, line=dict(color="white", width=1)),
                name="A · Cantidad ofrecida (lo que se vende)"))
            fig.add_trace(go.Scatter(
                x=[R["Qd"]], y=[ptecho], mode="markers+text",
                text=["B"], textposition="bottom left",
                textfont=dict(size=15, color="#dbeafe"),
                marker=dict(color=COL_DEM, size=12, line=dict(color="white", width=1)),
                name="B · Cantidad demandada (lo que se quiere)"))
            # Brecha de escasez entre A y B
            fig.add_trace(go.Scatter(
        ...
        st.caption("**A** · Cantidad ofrecida al precio tope (lo que realmente se "
                   "vende)   |   **B** · Cantidad demandada (lo que se querría "
                   "comprar)   |   **C** · Equilibrio libre")
